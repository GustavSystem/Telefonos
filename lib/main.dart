    ...
    try {
      ...
      if (kIsWeb) {
        ...
        try {
          ...
          print("Intentando cargar CSV desde URL...");
          final response = await HttpClient().getUrl(
              Uri.parse('/assets/MATERNO-2025.csv'));
          final httpResponse = await response.close();
          data = await httpResponse.transform(utf8.decoder).join();
          print("CSV cargado desde URL correctamente");

          // Si llegamos aquí, intentamos convertir el CSV real
          try {
            List<List<dynamic>> datosReales = const CsvToListConverter()
                .convert(data);
            ...
            print('Error al convertir CSV real: $csvError - manteniendo datos de prueba');
          }
        } catch (webError) {
          print('Error al cargar CSV en web: $webError - manteniendo datos de prueba');
        }
      } else {
